...
def vqt(qhbm, model, num_samples, hamiltonian, beta):
  ...
  def loss(trainable_variables):
    ...
    expectation = tf.squeeze(
        qhbm.q_inference.expectation(model.circuit, bitstrings, counts, hamiltonian), -1)
    entropy = qhbm.e_inference.entropy()

    def grad(grad_y, variables=None):
      with tf.GradientTape() as tape:
        tape.watch(model.circuit.trainable_variables)
      ...
      grad_qnn = tape.gradient(beta_expectation, model.circuit.trainable_variables)
      grad_qnn = [grad_y * g for g in grad_qnn]

      with tf.GradientTape() as tape:
        tape.watch(model.energy.trainable_variables)
        energies = model.energy(bitstrings)
      energy_jac = tape.jacobian(energies, model.energy.trainable_variables)
