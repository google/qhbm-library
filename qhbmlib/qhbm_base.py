...
class QHBM(tf.Module):
  ...
  def sample_bitstrings(self, num_samples):
    """Returns unique bitstrings and counts from this QHBM's classical dist.
        ...
        Args:
          num_samples: number of bitstrings to sample from the classical
            probability distribution.

        Returns:
          unique_samples:
          counts: 1D `tf.Tensor` of dtype `tf.int32` such that `counts[i]` is
          the
  ...
  def sample_state_circuits(self, num_samples):
    """Returns tensor of circuits generating pure state samples from this QHBM.
        ...
        Returns:
          circuit_samples: 1-D `tf.Tensor` containing the string representations
          of
              the circuits to generate pures state samples from this QHBM.
          bitstring_samples:
          counts: 1-D `tf.Tensor` of type `tf.int32` such that `counts[i]` says
          how
  ...
  def sample_unresolved_state_circuits(self, num_samples):
    ...
    circuit_samples = tfq.resolve_parameters(tiled_u_model, self.bit_symbols,
                                             bitstring_samples)
    return circuit_samples, bitstring_samples, counts
  ...
  def sample_pulled_back_bitstrings(self, circuit_samples, counts):
    """Returns samples from the pulled back data distribution.
        ...
        pulled back data density matrix. Then, the requested number of
        bitstrings
  ...
  def energy_and_energy_grad(self, bitstring):
    """Calculates the energy its derivative given a bitstring.
        ...
        Args:
          bitstring: `tf.Tensor` of type `tf.int8` which represents the
            bitstring.

        Returns:
          energy: The energy of this bitstring against the QHBMs energy
          function.
          grad: The derivative of `energy` with respect to the `thetas`
          parameters
            of this QHBM.
        """
...
class ExactQHBM(QHBM):
  ...
  def __init__(self, initial_thetas, energy, sampler, initial_phis,
    ...
    super().__init__(initial_thetas, energy, sampler, initial_phis,
                     phis_symbols, u, name)
  ...
  def all_energies(self):
    ...
    return tf.vectorized_map(lambda x: self.energy_function(self.thetas, x),
                             self.all_strings)
  ...
  def entropy_function(self):
    """Calculates the entropy of this QHBM.
        ...
        Returns:
          Scalar `tf.Tensor` of DType `tf.float32` which is the entropy of the
          EBM.
        """
  ...
  def eigvals(self):
    """Returns the eigenvalues of the density matrix this QHBM represents.
        ...
        The eigenvalues are the probabilities of the EBM, which are the
        exponentials
