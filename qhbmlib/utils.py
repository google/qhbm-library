...
def bitstrings_to_integers(bitstrings):
  """Converts bitstrings to integers.

  Assumes left most bit is most significant.
  Note: only works on up to 63 bits.
  ...
  exponents = tf.reverse(tf.range(bitwidth, dtype=tf.int64), [0])
  exponent_twos = tf.math.pow(twos, exponents)
  masked_twos = tf.cast(bitstrings, tf.int64) * exponent_twos
  return tf.reduce_sum(masked_twos, 1)


def integers_to_bitstrings(integers, bitwidth):
  """Converts integers to bitstrings.
  ...
  shifted_integers = tf.bitwise.right_shift(expanded_integers, shift_amounts)
  return tf.math.floormod(shifted_integers, 2)
