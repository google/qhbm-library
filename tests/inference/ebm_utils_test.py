...
class RelaxedCategoricalTest(tf.test.TestCase):
  ...
  def test_relaxed_categorical_probabilities(self):
    ...
    category_bitstrings = tf.constant(
        ...
        dtype=tf.int8)
    num_unique = 4
    categorical_prob_contributions = tf.constant(
        [3.0 / 7, 0.0, 1.0 / 7, 0.0, 0.0, 0.0, 2.0 / 7, 1.0 / 7])

    uniform_prob_contributions = tf.constant([1.0 / 8] * 8)
    ratio = inference.ebm_utils._relaxed_categorical_ratio(num_bits, num_unique)
    expected_probabilities = ratio * categorical_prob_contributions + (
        1.0 - ratio) * uniform_prob_contributions
  ...
  def test_relaxed_categorical_samples(self):
    ...
    relaxed_probabilities = tf.cast(counts_relaxed,
                                    tf.float32) / num_relaxation_samples
    ...
    def relaxed_prob_choice(bitstring):
      ...
      tiled_bitstring = tf.tile(
          tf.expand_dims(bitstring, 0), [num_unique_relaxed, 1])
      relaxed_matches = tf.math.reduce_all(
          tf.math.equal(tiled_bitstring, unique_relaxed), 1)
    ...
    actual_probabilities = tf.map_fn(
        relaxed_prob_choice,
        all_bitstrings_tensor,
