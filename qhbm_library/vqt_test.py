...
class VQTTest(tf.test.TestCase):
  ...
  def test_loss_consistency(self):
    ...
    test_qhbm = qhbm_base.ExactQHBM(
        self.initial_thetas, energy, sampler,
        self.initial_phis, self.raw_phis_symbols, self.u, self.name)
    ...
    for _ in range(num_random_hamiltonians):
      ...
      for beta in tf.constant([0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.4]):
        loss_estimate = vqt_base.vqt_loss(test_qhbm, num_samples, beta, tf_ham)
        loss_exact = vqt_base.exact_vqt_loss(test_qhbm, num_samples, beta,
                                             tf_ham)
...
class VQTLossTest(tf.test.TestCase):
  ...
  def test_zero_grad(self):
    ...
    for num_qubits in [1, 2]:
      ...
      loss = vqt.vqt_loss(model_qhbm, model_samples, sub_term_energy_func)
      thetas_grad = vqt.vqt_loss_thetas_grad(
          model_qhbm, model_samples, sub_term_energy_func)
      phis_grad = vqt.vqt_loss_phis_grad(
          model_qhbm, model_samples, sub_term_energy_func, eps=.02)
      print("Current num qubits: {}".format(num_qubits))
      self.assertAllClose(
          loss, -target_qhbm.log_partition_function(target_qhbm.thetas),
          atol=1e-2)
  ...
  def train_independent_multi_input(self):
    ...
    raw_dm_2 = tf.linalg.LinearOperatorFullMatrix(
        qhbm_base.approximate_density_matrix(qhbm_2, target_samples))
    target_dm = tf.linalg.LinearOperatorKronecker(
        [raw_dm_1, raw_dm_2]).to_dense()
    ...
    target_loss = -1.0*tf.reduce_sum([
        qhbm_1.log_partition_function(qhbm_1.thetas),
        qhbm_2.log_partition_function(qhbm_2.thetas)
    ])
    ...
    @tf.function
    def vqt_train_step(step):
      print("retracing: vqt_train_step")
      thetas_gradients = vqt.vqt_loss_thetas_grad(
          model_qhbm, model_samples, sub_term_energy_func)
      phis_gradients = vqt.vqt_loss_phis_grad(
          model_qhbm, model_samples, sub_term_energy_func, eps=.02)
      optimizer.apply_gradients([(thetas_gradients, model_qhbm.thetas),
    ...
    def vqt_train(steps):
      for step in range(steps):
        vqt_train_step(tf.constant(step, dtype=tf.int64))
    vqt_train(train_steps)
    final_loss = vqt.vqt_loss(model_qhbm, model_samples, sub_term_energy_func)
    final_dm = qhbm_base.approximate_density_matrix(model_qhbm, model_samples)
  ...
  def test_train_overlapping_multi_input(self):
    ...
    target_samples = 30000
    raw_dm_2_0 = tf.linalg.LinearOperatorFullMatrix(
        qhbm_base.approximate_density_matrix(qhbm_2_0, target_samples))
    raw_dm_2_1 = tf.linalg.LinearOperatorFullMatrix(
        qhbm_base.approximate_density_matrix(qhbm_2_1, target_samples))
    ident_1 = tf.linalg.LinearOperatorFullMatrix(
        tf.divide(tf.cast(tf.linalg.diag(tf.ones([2])), tf.complex64),
                  tf.cast(2, tf.complex64)))
    dm_2_0 = tf.linalg.LinearOperatorKronecker(
        [raw_dm_2_0, ident_1]).to_dense()
    dm_2_1 = tf.linalg.LinearOperatorKronecker(
        [ident_1, raw_dm_2_1]).to_dense()
