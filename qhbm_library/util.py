...
def pure_state_tensor_to_density_matrix(pure_states, counts):
  ...
  thermal_state = tf.reduce_sum(prod * tf.cast(
      tf.expand_dims(tf.expand_dims(counts, 1), 2), tf.complex64), 0)
  return tf.math.divide(
      thermal_state, tf.cast(tf.reduce_sum(counts), tf.complex64))
...
def fidelity(rho, sigma):
  ...
  rho_sqrt = tf.linalg.matmul(tf.linalg.matmul(
      v_rho, tf.linalg.diag(tf.math.sqrt(e_rho))), tf.linalg.adjoint(v_rho))
...
def fast_fidelity(rho, sigma):
  ...
  v_rho_sqrt_e_rho = tf.multiply(v_rho,
                                 tf.tile(tf.expand_dims(sqrt_e_rho, axis=0),
                                         (tf.shape(v_rho)[0], 1)))
...
def _np_fidelity_internal(e_rho, v_rho, sigma):
  ...
  v_rho_sqrt_e_rho = tf.cast(
      tf.multiply(v_rho, tf.tile(tf.expand_dims(sqrt_e_rho, axis=0),
                                 (tf.shape(v_rho)[0], 1))),
      tf.complex128)
  rho_sqrt = tf.linalg.matmul(v_rho_sqrt_e_rho, tf.linalg.adjoint(v_rho))
  omega = tf.linalg.matmul(tf.linalg.matmul(
...
def log_partition_function(beta, h_num):
  ...
  h_eigs, _ = np.linalg.eigh(tf.cast(h_num, tf.complex128))  # h_eigs are real since h_num is Hermitian
...
def density_matrix_to_image(dm):
  ...
  return tf.reshape(tf.concat(
      [dm_real, my_zeros, dm_imag], 2), (1, total_edge, total_edge, 3))
...
def get_bit_sub_indices(qubits_vqt, qubits_j):
  ...
  qubits_vqt_tiled = tf.tile(tf.expand_dims(qubits_vqt, 0),
                             [tf.shape(qubits_j)[0], 1, 1])
