...
@tf.function
def vqt_loss(qhbm: qhbm_base.QHBM, num_samples: tf.Tensor,
             beta: tf.Tensor, hamiltonian: tf.Tensor):
  ...
  expectations = tf.reshape(
      tfq.layers.SampledExpectation()(
          ...
          repetitions=tf.expand_dims(counts, 1)), [num_circuits])
  energy_approx = tf.reduce_sum(expectations * prob_terms)
  return minus_entropy_approx + beta * energy_approx


@tf.function
def vqt_loss_thetas_grad(qhbm: qhbm_base.QHBM,
                         num_samples: tf.Tensor, beta: tf.Tensor,
  ...
  energies, energy_grads = tf.map_fn(
      qhbm.energy_and_energy_grad, bitstrings,
...
@tf.function
def vqt_loss_phis_grad(qhbm: qhbm_base.QHBM,
                       num_samples: tf.Tensor, beta: tf.Tensor,
                       hamiltonian: tf.Tensor):
  ...
  with tf.GradientTape() as tape:
    ...
    sub_energy_list = tf.reshape(
        tfq.layers.Expectation()(
            ...
            operators=tf.tile(hamiltonian, [num_circuits, 1])), [num_circuits])
    e_avg = tf.divide(
...
def _tiled_expectation(circuits: tf.Tensor, hamiltonian: tf.Tensor):
  ...
  return tf.reshape(
      tfq.layers.Expectation()(
          circuits, symbol_names=tf.constant([], dtype=tf.string),
          symbol_values=tf.tile(
              tf.constant([[]], dtype=tf.float32), [tf.shape(circuits)[0], 1]),
          operators=measurements), [num_circuits])


@tf.function
def exact_vqt_loss(qhbm: qhbm_base.ExactQHBM, num_samples: tf.Tensor,
                   beta: tf.Tensor, hamiltonian: tf.Tensor):
...
@tf.function
def exact_vqt_loss_thetas_grad(qhbm: qhbm_base.ExactQHBM,
                               num_samples: tf.Tensor, beta: tf.Tensor,
                               hamiltonian: tf.Tensor):
...
@tf.function
def exact_vqt_loss_phis_grad(qhbm: qhbm_base.ExactQHBM,
                             num_samples: tf.Tensor, beta: tf.Tensor,
                             hamiltonian: tf.Tensor):
...
@tf.function
def get_avg_sub_energy(full_y_samples_j, qhbm_energy_function, qhbm_thetas_j,
                       bit_sub_indices_j):
...
def build_sub_term_energy_func(qhbm_vqt, qhbm_hamiltonian_list):
  ...
  qhbm_thetas = [
    this_qhbm.thetas for this_qhbm in qhbm_hamiltonian_list
  ]
  qhbm_u_dagger = [
    this_qhbm.u_dagger for this_qhbm in qhbm_hamiltonian_list
  ]
  qhbm_phis_symbols = [
    this_qhbm.phis_symbols for this_qhbm in qhbm_hamiltonian_list
  ]
  qhbm_phis = [
    this_qhbm.phis for this_qhbm in qhbm_hamiltonian_list
  ]
  qhbm_energy_list = [
  ...
  return inner_build_sub_term_energy_func(
      qhbm_energy_list, qhbm_thetas, bit_sub_indices_list, qhbm_u_dagger,
      qhbm_phis_symbols, qhbm_phis)
...
def vqt_qhbm_loss(qhbm_vqt, num_vqt_samples, sub_term_energy_func):
  ...
  vqt_circuits, _, vqt_circuits_counts = qhbm.sample_state_circuits(
      qhbm_vqt.sampler_function, qhbm_vqt.thetas, qhbm_vqt.bit_and_u,
      qhbm_vqt.bit_symbols, qhbm_vqt.phis_symbols, qhbm_vqt.phis,
      num_vqt_samples)
  sub_energy_list = sub_term_energy_func(vqt_circuits, vqt_circuits_counts)
  e_avg = tf.divide(
      tf.reduce_sum(
          tf.cast(vqt_circuits_counts, tf.float32) * sub_energy_list),
      num_vqt_samples)
...
def vqt_qhbm_loss_thetas_grad(qhbm_vqt, num_vqt_samples, sub_term_energy_func):
  ...
  (
      vqt_circuits, vqt_bitstrings, vqt_circuits_counts
  ) = qhbm.sample_state_circuits(
      qhbm_vqt.sampler_function, qhbm_vqt.thetas, qhbm_vqt.bit_and_u, qhbm_vqt.bit_symbols,
      qhbm_vqt.phis_symbols, qhbm_vqt.phis, num_vqt_samples)
  ...
  negative = tf.divide(
      tf.reduce_sum(vqt_expanded_circuits_counts * prod_list, 0),
      num_vqt_samples)
...
def phis_grad_sub_func(
    i, qhbm_sampler, qhbm_thetas, qhbm_bit_and_u, qhbm_bit_symbols,
    qhbm_phis_symbols, qhbm_phis, num_vqt_samples,
    D, eps, sub_term_energy_func):
  p_axis = tf.one_hot(i, D, dtype=tf.float32)
  perturbation = p_axis * eps

  # Forward
  (
      vqt_circuits_forward, _, vqt_circuits_counts_forward
  ) = qhbm.sample_state_circuits(
      qhbm_sampler, qhbm_thetas, qhbm_bit_and_u, qhbm_bit_symbols,
      qhbm_phis_symbols, qhbm_phis + perturbation, num_vqt_samples)
  ...
  # Backward
  (
      vqt_circuits_backward, _, vqt_circuits_counts_backward
  )= qhbm.sample_state_circuits(
      qhbm_sampler, qhbm_thetas, qhbm_bit_and_u, qhbm_bit_symbols,
      qhbm_phis_symbols, qhbm_phis - perturbation, num_vqt_samples)
  sub_energy_list = sub_term_energy_func(
...
@tf.function
def vqt_qhbm_loss_phis_grad(
    qhbm_vqt, num_vqt_samples, sub_term_energy_func, eps=0.1):
  print("retracing: vqt_qhbm_loss_phis_grad")
  D = tf.shape(qhbm_vqt.phis)[0]
  return tf.map_fn(
      lambda x: phis_grad_sub_func(
          x, qhbm_vqt.sampler_function, qhbm_vqt.thetas, qhbm_vqt.bit_and_u,
          qhbm_vqt.bit_symbols, qhbm_vqt.phis_symbols, qhbm_vqt.phis,
          num_vqt_samples, D, eps, sub_term_energy_func),
      tf.range(D),
      fn_output_signature=tf.float32)
