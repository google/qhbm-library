...
class BernoulliTest(tf.test.TestCase):
  ...
  def test_energy_bernoulli(self):
    ...
    ref_energy = tf.reduce_sum(tf.math.log(
        tf.constant([11.0, 8.0, 2.0], dtype=tf.float32)))
    ...
    test_energy = energy_bernoulli(test_logits, test_bitstring)
    ref_energy = tf.reduce_sum(tf.math.log(tf.constant(
        [2.0, 2.7, 3.8], dtype=tf.float32)))-tf.math.log(1.0)-tf.math.log(2.8)
    self.assertAllClose(test_energy, ref_energy)
  ...
  def test_energy_bernoulli_gradients(self):
    ...
    """
    (energy_bernoulli, sampler_bernoulli, log_partition_bernoulli,
     entropy_bernoulli, num_nodes) = ebm.build_bernoulli(3, "test")
  ...
  def test_sampler_bernoulli(self):
    ...
    self.assertTrue(tf.reduce_any(tf.equal(
        tf.constant([0], dtype=tf.int8), bitstrings)))
    self.assertTrue(tf.reduce_any(tf.equal(
        tf.constant([1], dtype=tf.int8), bitstrings)))
    ...
    self.assertFalse(tf.reduce_any(tf.equal(
        tf.constant([0], dtype=tf.int8), bitstrings)))
    self.assertTrue(tf.reduce_any(tf.equal(
        tf.constant([1], dtype=tf.int8), bitstrings)))
...
class BoltzmannTest(tf.test.TestCase):
  ...
  def test_energy_boltzmann(self):
    ...
    test_energies = tf.map_fn(lambda x: energy_boltzmann(test_thetas, x),
                              all_strings, fn_output_signature=tf.float32)
  ...
  def test_sampler_boltzmann(self):
    ...
    def check_bitstring_exists(bitstring, bitstring_list):
      ...
      return tf.math.reduce_any(
          tf.reduce_all(tf.math.equal(bitstring, bitstring_list), 1))
    self.assertTrue(check_bitstring_exists(
    ...
    self.assertFalse(check_bitstring_exists(
        tf.constant([0, 0, 0], dtype=tf.int8), bitstrings))
    self.assertFalse(check_bitstring_exists(
        tf.constant([0, 0, 1], dtype=tf.int8), bitstrings))
    self.assertFalse(check_bitstring_exists(
