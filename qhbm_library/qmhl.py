...
@tf.function
def exact_qmhl_loss(qhbm_model: qhbm_base.ExactQHBM, target_circuits: tf.Tensor,
...
@tf.function
def exact_qmhl_loss_thetas_grad(qhbm_model: qhbm_base.ExactQHBM,
  ...
  expanded_counts_c = tf.cast(
      tf.tile(
          tf.expand_dims(counts_c, 1), [1, tf.shape(qhbm_model.thetas)[0]]),
      tf.dtypes.float32)
  _, e_grad_list_c = tf.map_fn(
      qhbm_model.energy_and_energy_grad, unique_samples_c,
      fn_output_signature=(tf.float32, tf.float32))

  # Get energy gradients for the pulled-back data bitstrings
  ragged_samples_pb = qhbm_model.sample_pulled_back_bitstrings(target_circuits,
                                                               target_counts)
  ...
  _, e_grad_list_pb = tf.map_fn(
      qhbm_model.energy_and_energy_grad, unique_samples_pb,
      fn_output_signature=(tf.float32, tf.float32))

  # Build theta gradients after reweighting
  e_grad_c_avg = tf.divide(tf.reduce_sum(
      expanded_counts_c * e_grad_list_c, 0),
                           tf.cast(tf.reduce_sum(counts_c), tf.float32))
  e_grad_pb_avg = tf.divide(tf.reduce_sum(
      expanded_counts_pb * e_grad_list_pb, 0),
                            tf.cast(tf.reduce_sum(counts_pb), tf.float32))
  return tf.math.subtract(e_grad_pb_avg, e_grad_c_avg)


@tf.function
def exact_qmhl_loss_phis_grad(qhbm: qhbm_base.ExactQHBM, op_tensor: tf.Tensor,
                              target_circuits: tf.Tensor,
...
def qmhl_loss(qhbm_model, thetas, target_density):
  ...
  expected = qhbm.pulled_back_energy_expectation(
      qhbm_model.energy_function, thetas, qhbm_model.u_dagger,
      qhbm_model.phis_symbols, qhbm_model.phis, target_density.circuits,
...
@tf.function
def qmhl_loss_thetas_grad(
    qhbm_model, num_model_samples, target_density):
  """Calculate thetas gradient of the QMHL loss of the model against the target.
  ...
  expanded_counts_c = tf.cast(
      ...
      tf.dtypes.float32)
  def loop(x):
    return qhbm.energy_and_energy_grad(qhbm_model.energy_function,
                                       qhbm_model.thetas, x)
  _, e_grad_list_c = tf.map_fn(loop, unique_samples_c,
                               fn_output_signature=(tf.float32, tf.float32))

  # Get energy gradients for the pulled-back data bitstrings
  ragged_samples_pb = qhbm.sample_pulled_back_bitstrings(
      qhbm_model.u_dagger, qhbm_model.phis_symbols, qhbm_model.phis,
      target_density.circuits, target_density.counts)
  # safe when all circuits have the same number of qubits
  all_samples_pb = ragged_samples_pb.values.to_tensor()
  unique_samples_pb, _, counts_pb = util.unique_with_counts(all_samples_pb)
  expanded_counts_pb = tf.cast(
      tf.tile(
          tf.expand_dims(counts_pb, 1), [1, tf.shape(qhbm_model.thetas)[0]]),
      tf.dtypes.float32)
  _, e_grad_list_pb = tf.map_fn(loop, unique_samples_pb,
                                fn_output_signature=(tf.float32, tf.float32))
...
@tf.function
def phis_grad_sub_func(i, qhbm_energy_function, qhbm_thetas, qhbm_u_dagger,
                       qhbm_phis_symbols, qhbm_phis, circuit_samples,
                       circuit_counts, num_phis, eps):
  """phis_grad_sub_func."""
  ...
  forward = qhbm.pulled_back_energy_expectation(
      qhbm_energy_function, qhbm_thetas, qhbm_u_dagger, qhbm_phis_symbols,
      qhbm_phis + perturbation, circuit_samples, circuit_counts)
...
@tf.function
def qmhl_loss_phis_grad(qhbm_model, target_density, eps=.1):
  """Calculate phis gradient of the QMHL loss of the model against the target.
  ...
  def loop(x):
    phis_grad_sub_func(
        x, qhbm_model.energy_function, qhbm_model.thetas, qhbm_model.u_dagger,
        qhbm_model.phis_symbols, qhbm_model.phis, target_density.circuits,
        target_density.counts, num_phis, eps)
