...
def unique_with_counts(input_bitstrings, out_idx=tf.int32):
  ...
  base = tf.bitwise.left_shift(mask,
                               tf.range(tf.cast(input_shape[1], dtype=tf.int64),
                                        dtype=tf.int64))
  ...
  y = tf.tensor_scatter_nd_update(y, tf.expand_dims(idx, axis=1),
                                  input_bitstrings)
...
class QHBM(tf.Module):
  ...
  def __init__(self, initial_thetas, energy, sampler,
               initial_phis, phis_symbols, u, name):
  ...
  def copy(self):
    return QHBM(
        self.thetas, self.energy_function, self.sampler_function, self.phis,
        [sympy.Symbol(s.decode("utf-8")) for s in self.phis_symbols.numpy()],
        tfq.from_tensor(self.u)[0], self.name)
  ...
  def sample_pulled_back_bitstrings(self, circuit_samples, counts):
    ...
    raw_samples = self.tfq_sample_layer(
        pulled_back_circuits, symbol_names=self.phis_symbols,
    ...
    num_samples_mask = tf.cast(
        (tf.ragged.range(counts) + 1).to_tensor(), tf.bool)
  ...
  def pulled_back_energy_expectation(self, circuit_samples, circuit_counts):
    ...
    e_list = tf.map_fn(
        lambda x: self.energy_function(self.thetas, x), unique_samples_pb,
        fn_output_signature=tf.float32)
    e_list_full = tf.multiply(counts_pb, e_list)
    average_energy = tf.divide(
        tf.reduce_sum(e_list_full), tf.reduce_sum(counts_pb))
...
class ExactQHBM(QHBM):
  ...
  def __init__(self, initial_thetas, energy, sampler,
    ...
    super().__init__(initial_thetas, energy, sampler,
                     initial_phis, phis_symbols, u, name)
    self.all_strings = tf.constant(
        list(itertools.product([0, 1], repeat=len(self.raw_qubits))),
        dtype=tf.int8)
  ...
  def unitary_matrix(self):
    ...
    return self.tfq_unitary_layer(
        self.u, symbol_names=self.phis_symbols,
        symbol_values=tf.expand_dims(self.phis, 0)).to_tensor()[0]
  ...
  def density_matrix(self):
    ...
    u_mul_probs = tf.multiply(
        unitary,
        tf.tile(
            tf.expand_dims(probabilities, axis=0), (tf.shape(unitary)[0], 1)))
  ...
  def fidelity(self, sigma: tf.Tensor):
    ...
    omega = tf.linalg.matmul(tf.linalg.matmul(
        rho_sqrt, tf.cast(sigma, tf.complex128)), rho_sqrt)
...
class QHBMQ(tf.Module):
  ...
  def __init__(
      self, p, initial_etas, initial_classical_thetas,
      eta_theta_symbols, energy, sampler, log_partition,
      entropy, initial_phis, phis_symbols, u, name):
    ...
    if isinstance(initial_etas, tf.Variable):
      ...
      self.etas = tf.Variable(
          tf.cast(initial_etas, tf.float32), dtype=tf.float32)
    else:
      raise TypeError(
    ...
    if self.etas.shape[0] != self.p:
      raise ValueError(
          "must have as many eta values as trotter steps.")
    ...
    if not isinstance(eta_theta_symbols, (list, tuple)) or not all(
        [isinstance(s, sympy.Symbol)
         for row_s in eta_theta_symbols
         for s in row_s]):
      raise TypeError(
    ...
    if (self.classical_thetas.shape[0] * self.etas.shape[0]
    ...
    self.phis_symbols = tf.constant(
        [str(s) for s in phis_symbols], dtype=tf.string)
    ...
    self.raw_qubits = sorted(u.all_qubits())
    raw_bit_circuit, raw_bit_symbols = util.build_bit_circuit(
        self.raw_qubits, name)
  ...
  def copy(self):
    return QHBM(
        self.p, self.etas, self.classical_thetas, self.eta_theta_symbols,
        self.energy_function, self.sampler_function,
        self.log_partition_function, self.entropy_function,
        self.phis, self.raw_phis_symbols, self.raw_u, self.name)
...
def get_eta_theta_vals(etas, classical_thetas):
  ...
  return tf.reshape(etas_tiled * classical_thetas_tiled,
                    [p * num_classical_thetas])
...
def sample_state_circuits(this_qhbm, num_samples):
  ...
  eta_theta_vals = get_eta_theta_vals(this_qhbm.etas,
                                      this_qhbm.classical_thetas)
  total_vals = tf.concat([eta_theta_vals, this_qhbm.phis], 0)
  total_symbols = tf.concat(
      [this_qhbm.eta_theta_symbols, this_qhbm.phis_symbols], 0)
  u_model_concrete = tfq.resolve_parameters(
  ...
  circuit_samples = tfq.append_circuit(
      bit_injector_samples, tf.tile(
          u_model_concrete, [tf.shape(counts)[0]]))
  return circuit_samples, bitstring_samples, counts
...
def sample_pulled_back_bitstrings(this_qhbm, circuit_samples, counts):
  ...
  pulled_back_circuits = tfq.append_circuit(
      circuit_samples, tf.tile(
          u_dagger_concrete, [tf.shape(circuit_samples)[0]]))
...
def pulled_back_energy_expectation(this_qhbm, circuit_samples, circuit_counts):
  ...
  e_list_full = tf.multiply(counts_pb, e_list)
  average_energy = tf.divide(tf.reduce_sum(e_list_full),
                             tf.reduce_sum(counts_pb))
  return average_energy
