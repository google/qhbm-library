...
def build_bernoulli(num_nodes, identifier):
  ...
  def energy_bernoulli(logits, bitstring):
    ...
    return tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(bitstring,
                                                                 logits))
  ...
  def sampler_bernoulli(thetas, num_samples):
    ...
    return tfp.distributions.Bernoulli(logits=thetas,
                                       dtype=tf.dtypes.int8).sample(num_samples)
  ...
  def entropy_bernoulli(thetas):
    ...
    return tf.reduce_sum(tfp.distributions.Bernoulli(
        logits=thetas, dtype=tf.dtypes.int8).entropy())
...
def build_boltzmann(num_nodes, identifier):
  ...
  def get_all_boltzmann_sub(num_nodes, identifier):
    flat_spins_mask = tf.cast(
        tf.reshape(
            tf.linalg.band_part(
                tf.ones([num_nodes, num_nodes]), 0, -1
    ...
    def energy_boltzmann(thetas, x_in):
      ...
      spins_outer = tf.matmul(tf.transpose(tf.expand_dims(spins, 0)),
                              tf.expand_dims(spins, 0))
    ...
    def all_energies(thetas):
      ...
      return tf.map_fn(lambda x: energy_boltzmann(thetas, x), all_strings, fn_output_signature=tf.float32)
...
def bits_to_spins(x, n_bits):
  ...
  return tf.subtract(
      tf.ones(n_bits, dtype=tf.int8),
      tf.constant(2, dtype=tf.int8) * x)
...
def get_swish_network(num_bits, num_layers):
  ...
  def swish_network(thetas, x):
    ...
    return this_final_layer(
        thetas[n_init_params + n_hidden_params_total:], hidden_func(
...
def get_ebm_functions(num_bits, energy_function, ident):
  ...
  def all_energies(thetas):
    ...
    return tf.vectorized_map(
        lambda x: energy_function(thetas, x),
        all_strings
    )
...
class BitstringKernel(tfp.mcmc.TransitionKernel):
  ...
  def one_step(self, current_state, previous_kernel_results):
    ...
    kernel_results = BitstringKernelResults(
        target_log_prob=-1.0*self.energy_function(
  ...
  def bootstrap_results(self, init_state, energy_function_params):
    kernel_results = BitstringKernelResults(
        target_log_prob=-1.0*self.energy_function(
            energy_function_params, init_state),
        log_acceptance_correction=tf.constant(0.0),
...
class MCMCBitstringKernel(tfp.mcmc.TransitionKernel):
  ...
  def bootstrap_results(self, init_state, energy_function_params):
    pkr = self.inner_kernel.bootstrap_results(
        init_state, energy_function_params)
...
def get_bernoulli_proposal_function(flip_prob, num_qubits):
  dist = tfp.distributions.Bernoulli(
      probs=[flip_prob]*num_qubits, dtype=tf.bool)
  @tf.function
...
def get_chain(
    proposal_function, energy_function, num_burnin_steps,
    num_steps_between_results, parallel_iterations):
  ...
  def run_chain(last_state, energy_function_params, num_steps):
    ...
    return tf.cast(tfp.mcmc.sample_chain(
